...
class OpticalCharacterRecognizer:
    def __init__(self):
        self.image_path = "/Users/annabardadym/Desktop/schadenfreude.png"
        self.preprocessing_type = "thresh"
    ...
    def check_thresholding(self):
        if self.preprocessing_type == "thresh":
            gray = cv2.threshold(self.gray, 0, 255,
                                 cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        # make a check to see if median blurring should be done to remove
        # noise
        elif self.preprocessing_type == "blur":
